...
const projects: Project[] = [
  {
    ...
    description:
      "Transforms raw SQL/DDL scripts into interactive Entity-Relationship Diagrams, streamlining database design reviews.",
    previewLoader: () =>
      import("../../assets/SQL_schema_visulizer.gif").then((module) => module.default),
    highlights: [
  ...
  },
  {
    ...
    description:
      "Immersive 3D virtual garden for exploring medicinal plants with AI chatbot and gamification.",
    previewLoader: () =>
      import("../../assets/Herbal_garden.gif").then((module) => module.default),
    highlights: [
  ...
  },
  {
    ...
    description:
      "AI-based digital trainer using computer vision to analyze real-time video feeds and detect body posture.",
    previewLoader: () =>
      import("../../assets/A4fitness.gif").then((module) => module.default),
    highlights: [
  ...
  },
  {
    ...
    description:
      "A full-stack event ticketing platform with real-time inventory management and secure transactions.",
    previewLoader: () =>
      import("../../assets/eventTicketBooking.gif").then((module) => module.default),
    highlights: [
  ...
  },
  {
    ...
    description:
      "Production-ready task platform featuring real-time collaboration and scalable service architecture.",
  ...
  },
  {
    ...
    description:
      "Full-stack solution featuring Supabase S3 storage and enterprise-grade security protocols.",
    previewLoader: () =>
      import("../../assets/Document_managment.gif").then((module) => module.default),
    highlights: [
...
const Projects = () => {
  ...
  const loadPreview = async (project: Project) => {
    if (!project.previewLoader || previewSrc[project.title] || loadingPreview[project.title]) return;

    setLoadingPreview((prev) => ({ ...prev, [project.title]: true }));
    try {
      const src = await project.previewLoader();
      setPreviewSrc((prev) => ({ ...prev, [project.title]: src }));
    } finally {
      setLoadingPreview((prev) => ({ ...prev, [project.title]: false }));
    }
  };
    ...
    <section id="projects" className="section-padding py-12">
      <div className="section-container">
        <motion.div
          ...
          className="mb-10"
        >
          <p className="text-primary font-medium text-xs tracking-widest uppercase mb-2">Projects</p>
          <h2 className="text-2xl sm:text-3xl font-bold text-foreground">Featured Work</h2>
        </motion.div>
            ...
            return (
              ...
              >
                <div className="w-full h-36 rounded-xl bg-secondary/60 mb-3 flex items-center justify-center overflow-hidden">
                  {project.previewLoader ? (
                    previewSrc[project.title] ? (
                      <img
                        src={previewSrc[project.title]}
                        alt={project.title}
                        loading="lazy"
                        decoding="async"
                      ...
                      <button
                        type="button"
                        className="text-xs px-3 py-1.5 rounded-full bg-primary/15 text-primary hover:bg-primary/25 transition-colors"
                        onClick={(event) => {
                          event.stopPropagation();
                          void loadPreview(project);
                        }}
                      >
                        {loadingPreview[project.title] ? "Loading..." : "Load preview"}
                      </button>
                    )
                  ) : (
